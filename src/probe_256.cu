/* ============================================================================
 * PROBE 256×256 — Stress-Response Forensics for GTX 1050
 *
 * Modified for 256×256 grid with 12 guardians (scaled from 194)
 * Target: Sustainable coherence at 40-60W on GTX 1050
 *
 ...
 *
 * Build:  nvcc -O3 -arch=sm_61 -o probe_256 probe_256.cu -lnvidia-ml -lpthread
 * ============================================================================ */
...
/* ---- Precipitation ------------------------------------------------------ */
#define RHO_THRESH      1.00022f    /* Just above current max density (1.00021) */
#define DRAIN_RADIUS    4           /* SCALED: 16 × 0.25 = 4 */
#define SINK_RADIUS     6           /* SCALED: 24 × 0.25 = 6 */
#define SINK_RATE       0.0003125f  /* SCALED: 0.005 × 0.0625 = 0.0003125 */
#define MAX_PARTICLES   13          /* SCALED: 194 × 0.0625 = 12.125 → 13 */
...
int main()
{
    ...
    /* ---- CSV ---- */
    FILE* csv = fopen("build/probe.csv", "w");
    ...
    while (1) {  /* Run forever - MAX_CYCLES limit removed */
