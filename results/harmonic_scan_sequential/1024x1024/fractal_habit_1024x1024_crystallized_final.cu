...
int main()
{
    ...
    for (int batch = 0; batch < TOTAL_BATCHES; batch++) {
        ...
        if (current_step % 10000 == 0 && current_step > 0) {
            save_crystallized_checkpoint(current_step, f0, d_rho, d_ux, d_uy, 0.0, 0.0, 0.0, 0.0, 0);
        }
        /* Run one batch */
    ...
    save_crystallized_checkpoint(100000, f0, d_rho, d_ux, d_uy, 0.0, 0.0, 0.0, 0.0, 0);
    cufftDestroy(plan);
