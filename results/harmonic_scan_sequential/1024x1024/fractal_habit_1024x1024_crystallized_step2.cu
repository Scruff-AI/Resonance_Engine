...
/* ---- Crystallization Structures (adapted from the-craw) ---------------- */
typedef struct {
...
void save_crystallized_checkpoint(int step, float* d_f, float* d_rho, 
    ...
    // Calculate sizes
    size_t f_size = 9 * 1024 * 1024 * sizeof(float);
    size_t field_size = 1024 * 1024 * sizeof(float);
...
int main()
{
    ...
    auto do_sample = [&](uint64_t step) {
        ...
        /* Update crystallization tracking variables */
