/* ============================================================================
 * 1 MILLION STEP TEST - Full evolution metrics
 * ============================================================================ */
...
/* ---- Protocol ------------------------------------------------------------ */
#define TOTAL_STEPS      1000000  // 1 MILLION steps
...
#define CRYSTAL_VERSION 0x01000003  // v1.0.3 for 1M test
...
void crystallize_state(int step, float* d_f, float* d_rho, float* d_ux, float* d_uy,
    ...
    char filename[256];
    sprintf(filename, "C:\\fractal_nvme_test\\1M_test\\crystal_%08d.crys", step);
    
    printf("[Crystal] Step %d: %.3f bits, slope %.2f, k=%d, noise=%d\n", 
           step, entropy, slope, peak_k, noise_injections);
    system("mkdir C:\\fractal_nvme_test\\1M_test 2>nul");
...
int main() {
    ...
    printf("  Expected runtime: ~3 minutes (based on 100k=18.5s)\n");
    printf("=======================================================================\n\n");
    ...
    for (int batch = 0; batch < TOTAL_BATCHES; batch++) {
        ...
        /* Crystallize every 100k steps (for evolution tracking) */
        if (current_step % 100000 == 0 && current_step > 0 && spectral_available) {
    ...
    if (sr_f.spectral_entropy > 6.0) {
        ...
        printf("  >>> Evolution complete, stable state reached <<<\n");
    } else {
        printf("\n  >>> LOW ENTROPY STATE (%.2f bits) <<<\n", sr_f.spectral_entropy);
        printf("  >>> System may need different parameters <<<\n");
    }
