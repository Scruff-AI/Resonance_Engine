/* ============================================================================
 * SHORT TEST 2 - WITH METABOLIC KICK (like the-craw)
 * 10k steps with stochastic noise injection
 * ============================================================================ */
...
/* ---- Metabolic Kick Parameters (from the-craw) -------------------------- */
#define OMEGA  1.85f  // Higher omega for more activity
#define NOISE_AMPLITUDE 0.05f
#define NOISE_INTERVAL 50  // Inject noise every 50 steps
...
#define CRYSTAL_VERSION 0x01000001  // v1.0.1 with metabolic kick
...
void crystallize_state(int step, float* d_f, float* d_rho, float* d_ux, float* d_uy,
    ...
    char filename[256];
    sprintf(filename, "C:\\fractal_nvme_test\\crystal_kick_%08d.crys", step);
    
    printf("[Crystal] Crystallizing step %d (noise injections: %d)...\n", step, noise_injections);
    ...
    printf("[Crystal] Done: %.2f MB | Entropy: %.3f | Slope: %.2f | Peak k: %d | Noise: %d\n",
           (sizeof(header) + data_size) / (1024.0f * 1024.0f), entropy, slope, peak_k, noise_injections);
}
...
int main() {
    ...
    printf("    - Noise amplitude: %.3f\n", NOISE_AMPLITUDE);
    printf("    - Noise interval: %d steps\n", NOISE_INTERVAL);
    printf("    - Omega: %.2f (higher = more activity)\n", OMEGA);
