/* ============================================================================
 * FRACTAL HABIT 1024×1024 - CRYSTALLIZED VERSION (SIMPLE)
 * 
 * Adapts the-craw's crystallization approach to our 1024×1024 system.
 * Simple, working implementation - no over-engineering.
 * ============================================================================ */
...
/* ---- Crystallization Header (from the-craw) ---------------------------- */
typedef struct {
...
#define CRYSTAL_VERSION 0x01000000
...
void crystallize_state(int step, float* d_f, float* d_rho, float* d_ux, float* d_uy,
                       ...
                       double total_energy, uint32_t peak_k) {
    char filename[256];
    sprintf(filename, "C:\\fractal_nvme_test\\crystal_%08d.crys", step);
    
    printf("[Crystal] Crystallizing step %d...\n", step);
    ...
    char annotation[128];
    if (entropy > 6.0) {
        sprintf(annotation, "HIGH-ENTROPY: %.2f bits, slope %.2f, k=%d", entropy, slope, peak_k);
    } else if (entropy > 4.0) {
        sprintf(annotation, "MODERATE ENTROPY: %.2f bits", entropy);
    } else {
        sprintf(annotation, "LOW ENTROPY: %.2f bits", entropy);
    }
    ...
    printf("[Crystal] Done: %.2f MB | Entropy: %.3f | Slope: %.2f | Peak k: %d\n",
           (sizeof(header) + data_size) / (1024.0f * 1024.0f), entropy, slope, peak_k);
}
...
int main() {
    ...
    /* Spectral tracking for crystallization */
    ...
    printf("  sam |     step  | Velocity         | Density                       | Power\n");
    printf("  ----|-----------|------------------|-------------------------------|------\n");

    for (int batch = 0; batch < TOTAL_BATCHES; batch++) {
        int current_step = batch * STEPS_PER_BATCH;
        
        /* Crystallize every 10k steps */
    ...
    if (spectral_available) {
        crystallize_state(100000, f0, d_rho, d_ux, d_uy,
                        current_entropy, current_slope, current_kx0_frac,
                        current_total_energy, current_peak_k);
    }
    ...
    printf("  Velocity: H = %.4f bits (%.4f norm)  slope = %+.3f\n",
           sv_f.spectral_entropy, sv_f.spectral_entropy / H_max, sv_f.slope);
    printf("  Density:  H = %.4f bits (%.4f norm)  slope = %+.3f  kx0 = %.1f%%\n",
           sr_f.spectral_entropy, sr_f.spectral_entropy / H_max, sr_f.slope, sr_f.kx0_frac * 100);
    
    if (sr_f.spectral_entropy > 6.0) {
        printf("\n  >>> HIGH-ENTROPY CRYSTALLIZATION ACHIEVED <<<\n");
    } else if (sr_f.spectral_entropy > 4.0) {
